 ...
 * Do NOT display any console messages when this function is used.
 */
string ASConsole::formatString(string code)
{
	stringstream in;
	string out = "";
	// FileEncoding encoding = readFile(fileName_, in);

	//set code string in the input stream
	in << code;
...
string format_string(string code)
{
	ASFormatter formatter;
	...
	vector<string> argvOptions;
	argvOptions.push_back(string("--style=allman"));
	argvOptions.push_back(string("--indent=tab"));
	argvOptions.push_back(string("--pad-oper"));
	argvOptions.push_back(string("--indent-switches"));
	g_console->processOptions(argvOptions);

	string formated_code =  g_console->formatString(code);
...
extern "C"
{
	char* js_format_string(char* code)
	{
		string formated_code = format_string(code);
		char *formated_c_string=new char[formated_code.size() + 1];
		memcpy(formated_c_string, formated_code.c_str(), formated_code.size());
    formated_c_string[formated_code.size()] = 0;

		return formated_c_string;
	}
...
{
	string test_string = "int main(void)\n{int bla; if(true){char bla = \"hello\"}}\n";
	cout << test_string;
	string output_string = format_string(test_string);
	cout << output_string;
